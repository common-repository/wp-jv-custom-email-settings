 ...
 * Text Domain: wp-jv-custom-email-settings
 */
...
function wp_jv_ces_admin_init() {
	...
	add_action( 'delete_post', 'wp_jv_ces_delete_log_entry_on_post_delete', 10 );
}
//Initialize settings
...
add_action( 'admin_enqueue_scripts', 'wp_jv_ces_load_js_methods' );
...
function wp_jv_ces_notification_log_menu_page() {
	if ( !current_user_can( 'edit_posts' ) )  {
		wp_die( __( 'You do not have sufficient permissions to access this page.', 'wp-jv-custom-email-settings' ) );
	}
	//#TODO: add filter/search
	echo '<div class="wrap">';
	$wp_jv_ces_notification_log_table = new WP_JV_CES_List_Table();
	$wp_jv_ces_notification_log_table -> screen = convert_to_screen( null );
...
class WP_JV_CES_List_Table extends WP_List_Table {
	...
	function __construct( $args = array()){
		$args = wp_parse_args($args,  array(
			'singular'  => __( 'Notification email sent', 'wp-jv-custom-email-settings' ),     //singular name of the listed records
			'plural'    => __( 'Notification emails sent', 'wp-jv-custom-email-settings' ),   //plural name of the listed records
			'ajax'      => false,
			'screen'	=> null
			));
	}
...
function wp_jv_ces_prepare_mail_content($post_id, $user_id){
	...
	$content='<html><body>'.wp_jv_ces_resolve_tags($template,$post_id, $user_id).'</body></html>';
	return $content;
}
...
function wp_jv_ces_send_mail($post_id){
	...
	foreach ($recipients as $value) {
		...
		if (empty($mail_subject)|| empty($mail_content)) {
			...
			//Send mail
			$mail_status = wp_mail($value->display_name.'<'.$value->user_email.'>', $mail_subject, $mail_content );
			//Reinstate mail type
...
function wp_jv_ces_delete_log_entry_on_post_delete( $post_id ) {
    global $wpdb;
    if ( $wpdb->get_var( $wpdb->prepare( 'SELECT post_id FROM '.$wpdb->prefix.'jv_notification_log WHERE post_id = %d', $post_id ) ) ) {
        $wpdb->query( $wpdb->prepare( 'DELETE FROM '.$wpdb->prefix.'jv_notification_log WHERE post_id = %d', $post_id ) );
    }
}
